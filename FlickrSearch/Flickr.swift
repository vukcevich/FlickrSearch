...
//let apiKey = "e737c63bdef5cb28bb3d2790fb2c7768"
let apiKey = "6593783efea8e7f6dfc6b70bc03d2afb"
 ...
 // let processingQueue = OperationQueue()
  ...
  func searchFlickrForTerm(_ searchTerm: String, completion : @escaping (_ results: FlickrSearchResults?, _ error : NSError?) -> Void) {
    ...
    guard let searchURL = flickrSearchURLForSearchTerm(searchTerm) else {
      let APIError = NSError(domain: "FlickrSearch", code: 0, userInfo: [NSLocalizedFailureReasonErrorKey:"Unknown API response-1"])
    ...
    URLSession.shared.dataTask(with: searchRequest, completionHandler: { (data, response, error) in
      ...
      if let _ = error {
        let APIError = NSError(domain: "FlickrSearch", code: 0, userInfo: [NSLocalizedFailureReasonErrorKey:"Unknown API response-2"])
      ...
      guard let _ = response as? HTTPURLResponse,
        let data = data else {
          let APIError = NSError(domain: "FlickrSearch", code: 0, userInfo: [NSLocalizedFailureReasonErrorKey:"Unknown API response-3"])
          OperationQueue.main.addOperation({
            completion(nil, APIError)
          })
          return
      ...
      do {
        ...
        let stat = resultsDictionary["stat"] as? String else {
          ...
          let APIError = NSError(domain: "FlickrSearch", code: 0, userInfo: [NSLocalizedFailureReasonErrorKey:"Unknown API response-4"])
          OperationQueue.main.addOperation({
            completion(nil, APIError)
          })
          return
        ...
        case "fail":
          ...
          return
        default:
          let APIError = NSError(domain: "FlickrSearch", code: 0, userInfo: [NSLocalizedFailureReasonErrorKey:"Unknown API response-5"])
          OperationQueue.main.addOperation({
            completion(nil, APIError)
          })
          return
